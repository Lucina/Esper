...
namespace Esper.Misaka {
    ...
    internal class MisakaEntryIndex : IReadOnlyDictionary<string, Location> {
        ...
        internal static MisakaEntryIndex Create(IDictionary<string, Location> collection) {
            ...
            var len = collection.Sum(kvp => kvp.Key.Length);
        ...
        public bool ContainsKey(string key) {
            ...
            while (min <= max) {
                var mid = (min + max) / 2;
                var pos = _bufLoc[mid];
                var len = _bufLoc[mid + 1] - pos;
                var cmp = MemoryExtensions.CompareTo(baseSpan.Slice(pos, len), strKey,
