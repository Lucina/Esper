...
namespace Esper.Misaka {
    internal static class BlockUtil {
        ...
        internal static List<Location> WriteData(
            ...
            var cutStream = hashBlocks ? new CutStream() : null;
            using var zsa = hashBlocks
                ? new ZstandardStream(cutStream, CompressionMode.Decompress, true)
                : null;
            long tmpTar;
            var locations = new List<Location>();
            using var zs = new ZstandardStream(targetStream, ZstandardStream.MaxCompressionLevel, true);
            var currentBlock = 0;
            var currentBlockPosition = targetStream.Position;
            var currentBlockOffset = 0;
            blockList.Add(currentBlockPosition);
            foreach (var entryFunc in inputs) {
                var (stream, keepOpen, enforcedOffsetNullable) = entryFunc.Invoke();
                try {
                    var targetBlockOffset = currentBlockOffset;
                    // enforce offset
                    if (enforcedOffsetNullable is { } enforcedOffset) {
                        ...
                        if (currentBlockOffset > enforcedOffset) {
                            // fill block
                            if (enforceBlockFill) {
                                var left = BlockSize - currentBlockOffset;
                                while (left > 0) {
                                    var read = Math.Min(left, BufferSize);
                            ...
                            blockHashList?.Add(Hash(hasher, zsa, cutStream, targetStream, currentBlockPosition,
                    ...
                    while (true) {
                        ...
                        var len = WriteBlock(zs, currentBlockOffset, targetBlockOffset, out var resultBlockOffset,
                            stream);
            ...
            var endLeft = BlockSize - currentBlockOffset;
            while (endLeft > 0) {
                var read = Math.Min(endLeft, BufferSize);
                zs.Write(BlankBuffer, 0, read);
                endLeft -= read;
            }
        ...
        private static long Hash(Blake2BHashAlgorithm hasher, ZstandardStream zstandardStream, CutStream cutStream,
            Stream stream, long offset, long length) {
            zstandardStream.Reset();
            cutStream.Set(stream, offset, length);
            var hash = MemoryMarshal.Read<long>(hasher.ComputeHash(zstandardStream));
        ...
        private static int WriteBlock(ZstandardStream zstd, int currentBlockOffset, int targetBlockOffset,
            ...
            try {
                ...
                if (currentBlockOffset < targetBlockOffset) {
                    // pad block
                    var left = targetBlockOffset - currentBlockOffset;
                    while (left > 0) {
                        var read = Math.Min(left, BufferSize);
                        zstd.Write(BlankBuffer, 0, read);
                        left -= read;
                ...
                while (true) {
                    var read = sourceStream.Read(buf, 0, Math.Min(BufferSize, BlockSize - resultBlockOffset));
                    if (read == 0) break;
        ...
        internal class Reader : IDisposable {
            ...
            private readonly CutStream _cutStream;
            ...
            public Reader(Stream baseStream, IReadOnlyList<long> blockList) {
                ...
                _cutStream = new CutStream();
                _decompressor = new ZstandardStream(_cutStream, CompressionMode.Decompress, true);
            }

            public void FillStream(Location location, Stream targetStream) {
                var block = location.Block;
                var blockOffset = location.Offset;
                var left = location.Length;
                //_decompressor.Reset();
                while (left > 0) {
                    _cutStream.Set(_baseStream, _blockList[block], _blockList[block + 1] - _blockList[block]);
                    _decompressor.Reset();
            ...
            public byte[] GetArray(Location location) {
                ...
                while (left > 0) {
                    _cutStream.Set(_baseStream, _blockList[block], _blockList[block + 1] - _blockList[block]);
                    _decompressor.Reset();
                    var read = ReadBlock(_decompressor, true, blockOffset, res, ofs, left);
                    left -= read;
                    ofs += read;
                    blockOffset = 0;
                    block++;
            ...
            private static int ReadBlock(ZstandardStream zstd, int blockOffset, Stream targetStream,
                ...
                try {
                    // Skip blockOffset
                    var skipLeft = blockOffset;
                    while (skipLeft > 0) {
                        var read = zstd.Read(buf, 0, Math.Min(skipLeft, BufferSize));
                        if (read == 0) throw new EndOfStreamException();
                        skipLeft -= read;
                    }

                    // Read lower of bytes in block and bytes to decompress
                    var virtualNeed = Math.Min(BlockSize - blockOffset, bytesToDecompress);
                    var virtualLeft = virtualNeed;
                    while (virtualLeft > 0) {
                        var read = zstd.Read(buf, 0, (int) Math.Min(virtualLeft, BufferSize));
                        if (read == 0) throw new EndOfStreamException();
            ...
            private static int ReadBlock(ZstandardStream zstd, bool enableSkip, int blockOffset, byte[] targetBuffer,
                ...
                var buf = Shared.Rent(BufferSize);
                try {
                    var avail = targetBuffer.Length - offset;
                    ...
                    while (virtualLeft > 0 && avail > 0) {
                        var read = zstd.Read(targetBuffer, offset, (int) Math.Min(virtualLeft, avail));
                        if (read == 0) throw new EndOfStreamException();
                        virtualLeft -= read;
                        offset += read;
                        avail -= read;
            ...
            private class AutoStream : Stream {
                ...
                internal AutoStream(Location location, Stream stream, IReadOnlyList<long> blockList) {
                    ...
                    _stream = stream;
                    _cutStream = new CutStream {Reposition = true};
                    _blockList = blockList;
                    _cutStream.Set(_stream, _blockList[_block], _blockList[_block + 1] - _blockList[_block]);
                    _decompressor = new ZstandardStream(_cutStream, CompressionMode.Decompress, true);
                }
                ...
                public override int Read(byte[] buffer, int offset, int count) {
                    ...
                    var read = ReadBlock(_decompressor, _first, _blockOffset, buffer, offset, Math.Min(_left, count));
                    _first = false;
                    _left -= read;
                    _blockOffset += read;
